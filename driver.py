...
def prepare_crsp():
    if not os.path.exists('./2. backtesting/crsp data.csv'):
        if os.path.exists('./2. backtesting/csrp data.xlsx'):
            xlsx = pd.read_excel('./2. backtesting/csrp data.xlsx')
            xlsx.to_csv('./2. backtesting/crsp data.csv', index=False)

    if not os.path.exists('./crsp data'):
        os.mkdir('./crsp data')
        
    crsp = pd.read_csv('./2. backtesting/crsp data.csv')
...
df = pd.read_csv("./1. preprocessing/sp100.csv", header=0)
prepare_crsp()
...
#%%
    ...
    strategy.to_csv(f'./2. backtesting/eq_strategy_daily_return_{round((1-TOP)*100)}_{round(BOTTOM*100)}.csv') if equal_weight else strategy.to_csv(f'./2. backtesting/val_strategy_daily_return_{round((1-TOP)*100)}_{round(BOTTOM*100)}.csv')
...
equal_strategy = pd.read_csv(f'./2. backtesting/eq_strategy_daily_return_{round((1-TOP)*100)}_{round(BOTTOM*100)}.csv')
equal_strategy['Cumulative Return'] = (1 + equal_strategy['Daily Return']).cumprod() - 1
val_strategy = pd.read_csv(f'./2. backtesting/val_strategy_daily_return_{round((1-TOP)*100)}_{round(BOTTOM*100)}.csv')
...
plt.gca().yaxis.set_major_formatter(mticker.FuncFormatter(lambda y, _: '{:.0%}'.format(y)))
plt.legend()
plt.savefig(f'./2. backtesting/crsp_result_{round((1-TOP)*100)}_{round(BOTTOM*100)}.jpg')
plt.show()
plt.close()
